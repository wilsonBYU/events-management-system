...
attendeeController.get = async (req, res) => {
  ...
  */
  try {
    const result = await db.get("attendees", {
      _id: new ObjectId(req.params.attendee_id),
    });
    if (result.length > 0) {
      ...
      res
        .status(404)
        .json({ error: "No attendee was found with the given id" });
    }
  } catch (e) {
    throw { code: 500, message: e.message };
  }
...
attendeeController.getEvents = async (req, res) => {
  /**/
  try {
    const attendee = await db.get("attendees", {_id: new ObjectId(req.params.attendee_id)})
    const result = await db.get("events", {attendees: attendee[0].attendeeId});
    res.status(200).json(result);
  } catch (e) {
    throw { code: 500, message: e.message };
  }
...
attendeeController.addEvent = async (req, res) => {
  try {
    const attendee = await db.get("attendees", { _id: new ObjectId(req.params.attendee_id)})
    const result = await db.put("attendees", { _id: new ObjectId(attendee[0]._id) }, { $push: req.body });
    if (result.acknowledged) {
      await db.put("events", {eventId: req.body.eventId}, {$push: {attendees: attendee[0].attendeeId}})
    }  
    res.status(204).json(result);
  } catch (e) {
    console.error(e)
...
attendeeController.deleteEvent = async (req, res) => {
  try {
    const attendee = await db.get("attendees", { _id: new ObjectId(req.params.attendee_id)})
    const result = await db.put("attendees", { _id: new ObjectId(attendee[0]._id) }, { $pull: { eventId: req.params.event_id }});
    console.log(result, attendee[0].attendeeId, req.params.event_id)
    if (result.acknowledged) {
      await db.put("events", { eventId: req.params.event_id}, { $pull: {attendees: attendee[0].attendeeId}})
    }
    res.status(204).json(result);
  } catch (e) {
    throw { code: 500, message: e.message };
